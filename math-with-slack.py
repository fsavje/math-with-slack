...
parser.add_argument(
    '--mathjax-url',
    help=
    'Either a remote URL to download a MathJax release or a local path to a pre-downloaded MathJax release.',
...
def _diagnose_permission(e, app_path):
  ...
  if sys.platform == "win32":
    ...
    if isinstance(e, IOError) or isinstance(
        e, OSError) and (e.errno == errno.EPERM or e.errno == errno.EACCES):
      err_msg = (
          "Possibile fix:\n"
...
def find_candidate_app_files(path_globs,
                             filename="app*.asar",
...
if args.app_file is not None:
  ...
  if _platform == "darwin":
    candidate_app_asars = filter_candidates_app_files_by_arch(
        candidate_app_asars)
...
with open(app_path, mode='rb') as check_app_fp:
  (header_data_size, json_binary_size) = struct.unpack('<II',
                                                       check_app_fp.read(8))
...
try:
  ...
  exprint(
      'Cannot inject code to {}. Make sure the script has write permissions. {}'
      .format(app_path, diagnosis))
...
if 'MWSINJECT' in json_check['files']:
  ...
  try:
    ...
    diagnosis = _diagnose_permission(e, app_path)
    exprint(
        'Cannot remove previously injected code. Make sure the script has write permissions. '
        + diagnosis)
...
if os.path.isfile(app_backup_path):
  try:
    ...
    exprint(
        'Cannot remove old backup. Make sure the script has write permissions. '
        + diagnosis)
...
def read_package_json():
  ...
  binary = read_file_from_asar(int(package_json_desp['offset']),
                               int(package_json_desp['size']))
...
def dir_to_json_header(root_dir, initial_offset):
  ...
  for parent_abs, dirs, files in os.walk(root_dir):
    ...
    for file in files:
      ...
      rdict["files"][parent_components[-1]]["files"][file] = {
          'size': size,
          'offset': str(offset)
      }
      offset += size
...
ori_injected_file_size = json_header['files']['dist']['files'][
    injected_file_name]['size']
ori_injected_file_offset = int(
    json_header['files']['dist']['files'][injected_file_name]['offset'])
...
def get_reporthook():
  ...
  def reporthook(count, block_size, total_size):
    ...
    try:
      ...
      speed, percent = 0, 0
    sys.stdout.write(
        "\rDownloading MathJax...{:3d}%, {:3.1f} MB / {:3.1f} MB, {:6.1f} KB/s, {:3.1f} sec"
        .format(percent, report.progress_size / (1024 * 1024),
...
if os.path.isfile(args.mathjax_url):
  ...
  mathjax_tar_name, headers = urllib_request.urlretrieve(
      args.mathjax_url, [], get_reporthook())
...
if include_mathjax_inline:
  ...
  mathjax_json_header, append_file_paths = dir_to_json_header(
      mathjax_dir, ori_data_size + ori_injected_file_size + len(inject_code))
  json_header["files"]["node_modules"]["files"][
      "mathjax"] = mathjax_json_header["files"]["."]
...
json_header['files']['dist']['files'][injected_file_name][
    'size'] = ori_injected_file_size + len(inject_code)
json_header['files']['dist']['files'][injected_file_name]['offset'] = str(
    ori_data_size)
...
with open(app_backup_path, mode='rb') as ori_app_fp, \
  ...
  new_app_fp.write(
      struct.pack('<I', 8 + len(new_json_header) + new_json_header_padding))
  new_app_fp.write(
      struct.pack('<I', 4 + len(new_json_header) + new_json_header_padding))
  ...
  while ori_app_fp.tell() < copy_until:
    new_app_fp.write(ori_app_fp.read(min(65536,
                                         copy_until - ori_app_fp.tell())))
